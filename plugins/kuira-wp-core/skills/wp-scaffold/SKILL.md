...
> **Currency note (2026):** `@wordpress/scripts` is migrating its engine to
> `@wordpress/build` (esbuild-based, faster, auto-generates PHP registration). The
> commands below (`wp-scripts start`/`build`) stay the same through the transition —
> no action needed now, but prefer `@wordpress/scripts` over a hand-rolled webpack
> config so you inherit the upgrade for free.
