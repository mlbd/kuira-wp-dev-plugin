...
2. **Quality tooling** — header `Quality`, **multiSelect: true**
   - `Testing (wp-env + PHPUnit)` — *Local Docker WP + WP test suite (via wp-test).*
   - `E2E (Playwright)` — *Browser tests for the admin/front UI (via wp-e2e).*
   - `Static analysis (PHPStan)` — *Type/null/logic bug detection (via wp-analyze).*
   - `i18n / translation-ready` — *languages/ + .pot generation (via wp-i18n).*

3. **Ship & integrate** — header `Ship`, **multiSelect: true**
   - `Plugin Check (PCP)` — *wordpress.org compliance checking (via wp-plugin-check).*
   - `GitHub Actions CI` — *Lint + PHPStan + PHPUnit on PR (via wp-deploy).*
   - `WordPress.org auto-deploy` — *SVN deploy workflow on tag (via wp-deploy). Implies CI.*
   - `Live-WordPress MCP` — *Add an opt-in .mcp.json to drive a running WP site (commented; needs the user's site creds).*

4. **Git & commits** — header `Commits`, single-select
...
| E2E (Playwright) | `wp-e2e` | playwright.config.js + tests/e2e/ + npm scripts |
| Static analysis | `wp-analyze` | phpstan.neon.dist + WP stubs + composer `analyze` script |
...
| GitHub Actions CI | `wp-deploy` | `.github/workflows/ci.yml` (phpcs + phpstan + phpunit) |
| WordPress.org auto-deploy | `wp-deploy` | `.github/workflows/deploy.yml` + `.distignore` (lists required SVN secrets) |
| Live-WordPress MCP | this skill | commented `.mcp.json` the user fills with their site creds |
| Git & commits | this skill, Step 5b | git init + commit behavior per the user's choice |

> Only generate what was selected. CI must reference only scripts that exist — e.g.
> don't add a PHPStan step unless Static analysis was chosen, or an E2E job unless
> Playwright was. Keep the generated config honest.

> **Modular note.** `wp-new` ships in **kuira-wp-core**, but several skills it
> references live in companion plugins: **kuira-wp-quality** (`wp-test`, `wp-e2e`,
> `wp-playground`, `wp-analyze`) and **kuira-wp-ship** (`wp-deploy`, `wp-readme`,
> `wp-i18n`). The wizard contains enough knowledge to generate the selected config on
> its own — so if a referenced plugin isn't installed, **still generate it** and tell
> the user which plugin to add for the richer, dedicated workflow. Never block on a
> missing module.
