...
# Fast PHP syntax check first — catches fatals/parse errors cheaply, before the
# heavier phpcs run. A syntax error is more urgent than any style issue.
if command -v php >/dev/null 2>&1; then
  if ! LINT=$(php -l "$FILE" 2>&1); then
    echo "PHP syntax error in $FILE:" >&2
    echo "$LINT" | grep -iE "error|parse" | head -5 >&2
    exit 2
  fi
fi
